...
def add_uniform_noise(image):
    noise_percentage = 0.2  # 噪声占总数据的百分比
    noise_range = 0.0001  # 噪声的范围，可以根据需要调整

    # 生成与图像相同大小的随机噪声
    noise = (torch.rand(image.shape) - 0.5) * 2 * noise_range

    # 创建一个随机掩码，用于确定添加噪声的位置
    mask = (torch.rand(image.shape) < noise_percentage).float()
...
transform_cifar10_train = transforms.Compose([
    ...
    transforms.Lambda(add_uniform_noise)  # 添加均匀噪声
])
...
if not os.path.exists("record"):
    os.makedirs("record")
with open('record/results16.json', 'w') as file:
...
# # 画图
# # 更改
# Plot(NUM_EPOCHS, 'record/results16.json', 'images/images15')
